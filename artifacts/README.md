...
A command‑line utility (wrapped in `artifacts/spritecut/`) that extracts, cleans, resizes and centres every icon from a grid or auto‑detected sprite sheet into uniformly‑sized PNGs.
...
The command above produces 16 PNGs inside `assets/` (each 64 × 64 px, transparent background).
...
```bash
PYTHONPATH=/path/to/artifacts python -m spritecut.cli [args...]
```
...
| `--padding` | `-p` | Extra source pixels kept around each icon before scaling (default 4). | `--padding 8` |
...
| `--white-threshold` | – | Channel value (0‑255) above which a pixel is considered white background (default 244). | `--white-threshold 200` |
| `--alpha-threshold` | – | Alpha value (0‑255) below which a pixel is considered transparent (default 8). | `--alpha-threshold 16` |
| `--feather` | – | Softness of the cut edge, `0` = hard cut (default 8). | `--feather 0` |
| `--bg-color` | – | Replace the sheet background with this colour instead of white (`#RRGGBB`). | `--bg-color '#101014'` |
| `--color-tolerance` | – | Tolerance for `--bg-color` and near‑white greys (0‑255, default 24). | `--color-tolerance 30` |
| `--no-defringe` | – | Keep the whitish halo on semi‑transparent edge pixels (default off). | `--no-defringe` |
...
| **Auto‑detect icons on a dark background** | `python -m spritecut.cli sheet.png -s 48 --bg-color '#101014' --color-tolerance 30 -o out/` |
...
* **Grid vs auto‑detect** – If you know the exact rows × columns, use `-g`; it's faster and avoids mis‑detecting merged icons.
* **Background colour** – For dark sheets set `--bg-color` to the sheet's background hue and adjust `--color-tolerance` until all art is kept.
* **Merged parts** – Use `--merge-distance` (in pixels) to join blobs that belong to a single multi‑part icon.
* **Preview** – `--preview` writes a PNG with green boxes and reading‑order numbers; handy for debugging detection.

---

*Built from the `artifacts/spritecut/` pipeline. See the source files `cli.py`, `pipeline.py`, `imaging.py`, `labeling.py` for full implementation details.*